<script lang="ts">
  ...
  function segmentAtProgress(progress: number): number {
    ...
    for (let index = 0; index < normalized.segments.length; index += 1) {
      ...
      if (progress >= segment.startProgress && (progress < segment.endProgress || isLast)) {
        return index;
      }
    }
  ...
  function segmentTemplate(
    ...
    return {
      ...
      parameters: source.parameters ? structuredClone(source.parameters) : undefined,
    };
  }
  ...
  function splitSegment(progress: number) {
    ...
    if (!target || progress <= target.startProgress || progress >= target.endProgress) {
      return;
    }
  ...
  function getProgressFromPointer(event: PointerEvent, element: HTMLElement): number {
  ...
  function beginBoundaryDrag(boundaryIndex: number, event: PointerEvent) {
    ...
    const moveHandler = (moveEvent: PointerEvent) => {
      ...
      const normalized = normalizePiecewiseHeadingInterpolation(config);
      const progress = getProgressFromPointer(moveEvent, event.currentTarget as HTMLElement);
      const minGap = 0.0001;
  ...
  function handleTimelineClick(event: PointerEvent) {
    if (locked) return;
    const progress = getProgressFromPointer(event, event.currentTarget as HTMLElement);
    splitSegment(progress);
    notifyChange();
  }

  function updateSegment(index: number, patch: Partial<PiecewiseHeadingSegment>) {
    if (locked) return;
    const normalized = normalizePiecewiseHeadingInterpolation(config);
    const segment = normalized.segments[index];
    if (!segment) return;
  ...
  function setInterpolationType(index: number, interpolationType: PiecewiseHeadingInterpolationType) {
    const segment = normalizePiecewiseHeadingInterpolation(config).segments[index];
    if (!segment) return;

    const nextParameters = (() => {
      if (interpolationType === "constant") return { degrees: segment.parameters?.degrees ?? 0 };
      ...
      if (interpolationType === "facing-point") {
        return {
          point: segment.parameters?.point ? { ...segment.parameters.point } : { x: 0, y: 0 },
        };
      }
      return undefined;
    })();
...
<div class="space-y-3 rounded border border-neutral-700 bg-neutral-950/60 p-3 text-xs text-neutral-200">
  <div class="flex flex-wrap items-center justify-between gap-2">
    <div>
      ...
      <div class="text-neutral-400">Click the timeline to add a boundary. Drag markers to adjust segment breaks.</div>
    </div>
  ...
  {#if validationMessage}
    <div class="rounded border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[11px] text-amber-200">
      {validationMessage}
    </div>
  {/if}
      ...
      <div class="rounded border border-neutral-700 bg-neutral-900/80 p-2">
        ...
        <div class="mt-2 grid gap-2 sm:grid-cols-2">
          ...
          {#if segmentSupportsReverse(segment.interpolationType)}
            <label class="flex items-end gap-2 rounded border border-neutral-700 px-2 py-1 text-[11px] text-neutral-300">
              <input
                type="checkbox"
        ...
        {#if segment.interpolationType === "linear"}
          <div class="mt-2 grid gap-2 sm:grid-cols-2">
            <label class="space-y-1">
              <div class="text-[11px] text-neutral-400">Start heading (deg)</div>
              <input
                type="number"
                step="1"
                class="w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1 text-neutral-100"
        ...
        {:else}
          <div class="mt-2 text-[11px] text-neutral-400">This segment uses the path tangent. No extra parameters are required.</div>
        {/if}

        <div class="mt-2 text-[11px] text-neutral-400">
          {#if advancedMode}
            Progress: {segment.startProgress.toFixed(3)} to {segment.endProgress.toFixed(3)}
          {:else}
            Progress values are read-only here. Enable advanced mode to edit them manually.
          {/if}
        </div>
      </div>
    {/each}
  ...
  <div class="flex justify-end">
    <button class="rounded border border-neutral-700 px-3 py-1 text-[11px] text-neutral-100 hover:bg-neutral-800 disabled:opacity-40" on:click={() => {
      ...
      normalized.segments.push({
        ...newSegment,
        startProgress: normalized.segments[normalized.segments.length - 1].endProgress,
        endProgress: 1,
      });
      config = normalizePiecewiseHeadingInterpolation(normalized);
      notifyChange();
